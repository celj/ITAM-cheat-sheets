...
::: {.definition name="Nonlinear Programming Problem"}
Let $f : \R^n \to \R$, $g_1 : \R^n \to \R$, . . . , $g_m : \R^n \to \R$ be differentiable functions. The *NLP problem* is
...
::: {.definition name="Kuhn-Tucker Pair for NLP"}
A pair $\left(\bar{x}, \bar{\lambda}\right) \in \R^{n + m}$ is a *Kuhn-Tucker pair for NLP* if
...
::: {.definition name="Active Constraints"}
For any $x \in \R^n$, let $I(x) = \left\{ i = 1, \dots, m : g_i(x) = 0 \right\}$. Hence $I(x)$ is the set of *active constraints* at $x$.
:::

::: {.lemma}
Suppose that $\left( \bar{x}, \bar{\lambda} \right)$ is a Kuhn-Tucker pair for NLP. If $I\left(\bar{x}\right) = \O$, then $\nabla f\left(\bar{x}\right) = 0$. If $I\left(\bar{x}\right) \neq \O$, then $\displaystyle \nabla f\left(\bar{x}\right) = \sum_{i = 1}^m \nabla g_i\left(\bar{x}\right)\bar{\lambda}_i$.
:::
...
::: {.definition name="Linear Programming Problem"}
For $a_1, \dots, a_m, c \in \R^n$ and $b_1, \dots, b_m \in \R$, consider the following *LP problem*:

$$
\begin{align}
\min_{x \in \R^n} \quad & c \cdot x \\
...
::: {.definition name="Kuhn-Tucker Pair for LP"}
A pair $\left(\bar{x}, \bar{\lambda}\right) \in \R^{n + m}$ is a *Kuhn-Tucker pair for LP* if
...
::: {.theorem}
Suppose that $A = \left[ a_1 \mid \cdots \mid a_m \right]$ and $a_i \in \R^n$ for all $i = 1, \dots, m$. Then the set

$$
\pos A := \left\{ Au : u \in \R^m_+ \right\}
$$
...
::: {.theorem}
If $\bar{x}$ solves the LP problem, then there exists some $\bar{\lambda}$ such that $\left(\bar{x}, \bar{\lambda}\right) \in \R^{n + m}$ is a Kuhn-Tucker pair for LP.
:::

::: {.definition name="General Constraint Qualification"}
...
::: {.definition name="Cottle Constraint Qualification"}
A solution $\bar{x}$ to the NLP problem satisfies the *Cottle constraint qualification (CCQ) condition* if there exists some $z \in \R^n$ such that $\nabla g_i \left(\bar{x}\right) \cdot z > 0$ for all $i \in I\left(\bar{x}\right)$.
:::
...
::: {.definition name="Linear Independence Constraint Qualification"}
...
::: {.theorem}
Let $f : \R^n \to \R$ be differentiable. Then
...
::: {.theorem}
Suppose $\bar{x} \in \R^n$ satisfies the KT conditions for NLP. If $f$ is convex and each $g_i$ is concave, then $\bar{x}$ solves the NLP problem.
:::

::: {.definition name="Quasi-convex / Quasi-concave"}
A function $f : \R^n \to \R$ is *quasi-convex / quasi-concave* if the lower-contour set $\left\{ x \in \R^n : f(x) \leq \alpha \right\}$ / the upper-contour set $\left\{ x \in \R^n : f(x) \geq \alpha \right\}$ is convex for every $\alpha \in \R$.
:::
...
::: {.definition name="Pseudo-convex / Pseudo-concave"}
A differentiable function $f : \R^n \to \R$ is *pseudo-convex* if, for every $x$ and $y$, $\nabla f(x) \cdot (y - x) \geq 0$ implies $f(y) - f(x) \geq 0$. A differentiable function $f : \R^n \to \R$ is *pseudo-concave* if, for every $x$ and $y$, $\nabla f(x) \cdot (y - x) \leq 0$ implies $f(y) - f(x) \leq 0$.
:::
...
Consider the problem $\displaystyle \min_{x \in \R^n} F(x, \alpha)$ subject to $G_i (x, \alpha) \geq 0$ for all $i = 1, \dots, m$, where $\alpha \in \R^p$ and all functions are differentiable. Call this problem $P_\alpha$. Let $v(\alpha)$ be the value of $P_\alpha$.

::: {.definition name="Regular Solution"}
A vector $\bar{x} \in \R^n$ is a *regular solution to $P_\bar{\alpha}$* if there exist $\varepsilon > 0$ and $\hat{x} : B_\varepsilon (\bar{\alpha}) \to \R^n$ such that
...
::: {.remark name="Indirect Utility Function Properties"}
<br/>
...
::: {.remark name="Expenditure Function Properties"}
<br/>
...
::: {.theorem name="Roy's Identity"}
$$
x_i(p,y) = -\frac{\displaystyle \frac{\partial v}{\partial p_i}}{\displaystyle \frac{\partial v}{\partial y}}.
$$
:::

::: {.remark name="Relationships between utility maximization and expenditure minimization"}
<br/>
...
::: {.definition name="Strict Preference"}
...
::: {.definition name="Weak Preference"}
...
Let $2^X = \left\{ A \subseteq X : A \neq \O \right\}$. Any member of $2^X$ is a *menu*.
:::
...
::: {.corollary}
Let $R$ be a complete and transitive binary relation (i.e., a weak order) on $X \subseteq \R^n$. Define, as usual, $xPy$ if and only if $xRy$ and $y \notR x$. Recall that such $P$ is asymmetric and negatively transitive. Endow $X$ with the Euclidean metric. We say that $R$ admits a continuous utility representation if there exists a continuous function $u : X \to \R$ such that $xRy$ if and only if $u(x) \geq u(y)$.
:::

::: {.theorem}
Let $X \subseteq \R^n$ be convex and $R$ be a binary relation on $X$. $R$ is a continuous weak order if and only if it admits a continuous utility representation.
:::

::: {.proposition}
Let $e = (1, \dots, 1) \in \R^n$. If $R$ is a strictly monotone weak order and $\alpha$ and $\beta$ are scalars, then
$$
...
::: {.theorem}
Suppose $R$ is a strictly monotone and continuous weak order on $X = \R^n$. Then $R$ admits a continuous utility representation.
:::